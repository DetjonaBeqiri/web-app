...
if(empty($username) || empty($email) || empty($password)) {
    ...
    header("Location: ../log_in.php.php?error=invalidUserName");
}    else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header("Location: ../log_in.php?error=invalidEmail&username=".$username);
    exit();
}    else if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
    header("Location: ../log_in.php?error=invalidUsername&username=".$email);
    exit();
} else {
    $sql = "SELECT uidUsers FROM users WHERE uidUsers=?";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt, $sql)) {
        header("Location: ../log_in.php.php?error=sqlerror");
        exit();
        var_dump($conn);
    } else {
        ...
        if($resultCheck > 0) {
            header("Location: ../log_in.php?error=usertaken&email=" . $email);
            exit();
        } else {
            ...
            if(!mysqli_stmt_prepare($stmt, $sql)) {
                ...
                echo var_dump($conn);
                 ...
                 mysqli_stmt_execute($stmt);
                 header("Location: ../log_in.php?user-inserted");
                 exit();
             }
        }
    }
